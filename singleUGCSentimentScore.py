# -*- coding: utf-8 -*-
# 计算单条UGC情感强度值，程度副词根据程度副词词典指定权值wi，程序副词权值用于计算情感强度,w1*w2...
...
def single_ugc_senti_score(single_ugc):
    ...
    i = 0    # 记录扫描到列表中的词的位置，列表的下标从0开始，所以设置为0比较好
    ...
    for word in single_ugc:    # 逐词分析，统计句子中的正向情感词和负向情感词个数、并计算总得分
        ...
        if word in posdict:  # 如果是积极情感词
            ...
            num = 0  # 统计副词个数
            for w in single_ugc[positive_senti_wrod:i]:
                if is_adv(w):
                    num += 1
            ...
            positive_senti_wrod = i+1      # 记录情感词的位置变化
    ...
    if num_pos_senti_word > 0:
        ...
        # print("num_pos_senti_word",num_pos_senti_word)
...
single_comment = ['快递', '收到', '了', '外观', '挺', '好看', '使用', '了', '一会儿', '运行', '速度', '很', '流畅']
single_score = single_ugc_senti_score(single_comment)
print("single_UGC_sentiment_score:", single_score)
