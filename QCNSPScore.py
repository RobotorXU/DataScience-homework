...
# 导入否定词词典
inversedict = open(r'sentimentDict/notdict.txt', 'r').read().split()     # 权 值为-1


def is_adv(word):
    # 判断是否为程度副词
    if word in mostdict or word in verydict or word in moredict or word in ishdict or word in overdict\
            or word in insufficientdict:
        return 1
...
def qcnsp_score(single_ugc):
    # 求单条评论语句的情感倾向总得分，评论语句参数为列表形式
    p = 0                     # 记录扫描到列表中的词的位置，列表的下标从0开始，所以设置为0比较好
    positive_senti_word = 0     # 记录前一个情感词的位置，初始值为0，初始位置未必是情感词，但是不影响情感值的计算。
    ...
    for word in single_ugc:   # 逐词分析，统计句子中的正向情感词和负向情感词个数、并计算总得分
        ...
        if word in posdict:  # 如果是积极情感词
            ...
            num = 0  # 统计副词个数
            for w in single_ugc[positive_senti_word: p]:
                if is_adv(w):
                    num += 1
            ...
            positive_senti_word = p+1      # 记录情感词的位置变化
...
for i in comment:
    ...
    qcnsp_score0 = qcnsp_score(sComment)
    qcnsp_scores = qcnsp_scores + str(qcnsp_score0) + " "
outData = open(r'comments/UGC_50_QCNSPScore.txt', 'w', encoding='UTF-8')
outData.write(qcnsp_scores)
outData.close()
print("UGC_Sentiment_qcnsp_score:", qcnsp_scores)
t2 = time.time()
t = round(t2-t1, 2)
print("t:", t)
outData = open(r'comments/UGC_50_QCNSPScore.txt', 'a', encoding='UTF-8')
outData.write(str(t))
outData.close()
